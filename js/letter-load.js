function load() {
    for (let i = 0; i < letters.length; i++) {
        addEnvelope(letters[i].id, letters[i].title, letters[i].author || "jake")
    }
...
function addEnvelope(id, title, author) {
    let envs = document.getElementById('envelope-container')
    envs.innerHTML +=
        `<div class="envelope" data-id=${id}>
            <svg class="author-${author}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
...
function displayLetter(id) {
    ...
    if (letter.ps && letter.ps !== "") {
        content.innerHTML = content.innerHTML + `<p>${letter.ps}</p>`;
    }
